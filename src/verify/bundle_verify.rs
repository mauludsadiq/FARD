        ...
        match v {
            ...
            JsonVal::Int(n) => {
                ...
                if s.starts_with('0') && s.len() > 1 && !s.starts_with("0.") {
                    return Err("M5_CANON_NUM_LEADING_ZERO".into());
                }
                if s.ends_with(".0") {
                    return Err("M5_CANON_NUM_DOT0".into());
                }
                out.push_str(&s);
                Ok(())
            }
...
fn expect_only_keys(
    ...
    for k in obj.keys() {
        if !allow.contains(k.as_str()) {
            return Err(format!("M5_EXTRA_KEY {}", k));
        }
    }
    Ok(())
}
...
pub fn verify_bundle_outdir(outdir: &str) -> Result<(), String> {
    ...
    if !is_sha256(preimage_sha256) {
        return Err("M5_BAD_preimage_sha256".into());
    }
    ...
    if !trace_exists {
        return Err("M5_MISSING_FILE trace.ndjson".into());
    }
    if !modg_exists {
        return Err("M5_MISSING_FILE module_graph.json".into());
    }

    if ok {
        if !res_exists {
            return Err("M5_OK_MISSING_result.json".into());
        }
        if err_exists {
            return Err("M5_OK_FORBIDS_error.json".into());
        }
    } else {
        if !err_exists {
            return Err("M5_ERR_MISSING_error.json".into());
        }
        if res_exists {
            return Err("M5_ERR_FORBIDS_result.json".into());
        }
    }
    ...
    if ok {
        expected.insert("result.json".to_string());
    } else {
        expected.insert("error.json".to_string());
    }
    if ag_exists {
        expected.insert("artifact_graph.json".to_string());
    }
    ...
    for k in expected.iter() {
        if !seen.contains(k) {
            return Err(format!("M5_DIGESTS_MISSING_FILE {}", k));
        }
    }
    for k in seen.iter() {
        if !expected.contains(k) {
            return Err(format!("M5_DIGESTS_EXTRA_FILE {}", k));
        }
    }
    ...
    for name in expected.iter() {
        ...
        if !is_sha256(cid) {
            return Err(format!("M5_BAD_FILE_CID {}", name));
        }
        ...
        if want != cid {
            return Err(format!("M5_FILE_HASH_MISMATCH {}", name));
        }
    }
    ...
    if pre_cid != preimage_sha256 {
        return Err("M5_PREIMAGE_HASH_MISMATCH".into());
    }
